...
export class ConversationsProcessor {
  ...
  async handleCheckAbandoned(job: any) {
    ...
    this.logger.log(
      `[${tenantId}] Checking if conversation ${conversationId} is abandoned`,
    );
    ...
    try {
      ...
      if (
        ...
        const thresholdMs =
          process.env.NODE_ENV === 'development' ? 25000 : 23 * 60 * 60 * 1000;

        if (inactiveTimeMs >= thresholdMs) {
          // Update status to ABANDONED
          await this.conversationsService.abandonConversation(
            tenantId,
            conversation.phone,
          );
          ...
          const followUpMsg =
            "Hi! We noticed you were asking about an order earlier. Would you like to continue? Just reply here and we'll help you complete your purchase. 😊";
          ...
          this.logger.log(
            `[${tenantId}] Conversation ${conversationId} marked as ABANDONED. Cart recovery follow-up sent.`,
          );
        } else {
          this.logger.log(
            `[${tenantId}] Conversation ${conversationId} is still active. Skipping abandonment.`,
          );
        }
      } else {
        this.logger.log(
          `[${tenantId}] Conversation ${conversationId} is in stage ${conversation.currentStage}. Skipping abandonment.`,
        );
      }
    } catch (err: any) {
